...
func (parser *defaultParser) Translator() translation.Translator {
	return parser.translator
}

func (parser *defaultParser) AddError(
	e string,
	code int,
) {
	parser.mapper[e] = code
}

func (parser *defaultParser) Parse(
	value any,
	...
	for _, verr := range verrs {
		paramId, errorId, e := parser.convert(value, verr)
		env = env.
			WithError(
				envelope.NewEnvelopeError(parser.status, e).
					WithParamId(paramId).
					WithErrorId(errorId))
	}
...
func (parser *defaultParser) convert(
	value any,
	fieldError validator.FieldError,
) (int, int, error) {
	typeof := reflect.TypeOf(value)
	field, _ := typeof.FieldByName(fieldError.StructField())
	iParam := 0
	if param, ok := field.Tag.Lookup(parser.annotation); ok {
		iParam, _ = strconv.Atoi(param)
	}

	return iParam, parser.mapper[fieldError.Tag()], errors.New(fieldError.Translate(parser.translator))
}
